...
class Day14(input: Seq[String], isSample: Boolean)
  ...
  for xs <- paths do
    ...
    for node <- xs.tail do
      ...
      val dx = x2 - x
      val dy = y2 - y
      if dx == 0 then for $y <- y to y2 by dy.sign do arr(x)($y) = 1
      else for $x <- x to x2 by dx.sign do arr($x)(y) = 1
      cur = node
    ...
    returning {
      while true do
        ...
        while advance(p) != p do
          p = advance(p)
          if p._2 > paths.flatten.map(_._2).max then throwReturn(n)
        arr(p._1)(p._2) = 2
        n += 1
    }
    n
  ...
  override def run2: Any =
    ...
    for i <- 0 until 1000 do arr(i)(max) = 1

    var n = 0
    while arr(start._1)(start._2) == 0 do
      var p = start
      while advance(p) != p do p = advance(p)
      arr(p._1)(p._2) = 2
      n += 1
    n
