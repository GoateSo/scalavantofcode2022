...
class Day11(input : Seq[String]) extends Solution(input):
  ...
  def simulate(n : Int, fn : Long => Long): Long = 
    var (lvls, res) = (1 to n).foldLeft(  // each run, using levels & monkies as accumulator
      ...
      (acc, l) => // for each monkey, have them perform their throws
        (0 until mks.size).foldLeft(acc){
          case ((nlvls, nvals), i) => 
            ...
            val nvs = nvals.updated(i, List.empty)      // clear the current list
                           .updated(t, nvals(t) ++ tvs)             // add the new values to true/false
                           .updated(f, nvals(f) ++ fvs)
            (nlvls.updated(i, nlvls(i) + vs.size), nvs) // update the levels and return both 
        }
    }
    lvls.sorted.reverse.take(2).product // return the product of the top 2 levels
