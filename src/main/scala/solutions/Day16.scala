...
class Day16(input: Seq[String], isSample: Boolean)
  ...
  val spaths = pfr.map((a, _, _) => (graph.path("AA", a)._1, a))
  def simulate(cur: String, minutes: Int, on: Set[String], flow: Int = 0): Int =
    ...
    else if minutes <= 1 then flow + rate * minutes
    else
      val nmin = minutes - 1
      val nflow = flow + rate
      val on2 = on + cur
      val nrate = rate + rates(cur)
      val adjs = dists(cur).filter { case (s, l) =>
        !on(s) && minutes > l + 1
      }
      if adjs.isEmpty then nflow + nrate * nmin
      else
  ...
  override def run: Any =
    val res =
      for (w, node) <- spaths yield simulate(node, 30 - w.toInt, Set.empty)
    res.max
