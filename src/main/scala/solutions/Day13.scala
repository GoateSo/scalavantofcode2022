...
class Day13(input: Seq[String], isSample: Boolean)
  ...
  def isGood(xs: List[Any], ys: List[Any]): (Boolean, Boolean) =
    (xs, ys) match
      ...
      case (x :: xs, y :: ys) =>
        (x, y) match
          case (a: Int, b: Int) =>
            if a == b then isGood(xs, ys) else (a < b, true)
          case (a, b) =>
            val (i, e) = (a, b) match
  ...
  override def run: Any =
    ...
    val ys = xs.zipWithIndex.filter((a, b) => isGood(a._1, a._2)._1)
    ys.map(_._2 + 1).sum

  override def run2: Any =
    val xs = List(List(2)) +: List(List(6)) +: pairs
      .map(ls => ls.map(s => parse(s.toSeq)))
      .flatten
    // write( xs mkString "\n")
